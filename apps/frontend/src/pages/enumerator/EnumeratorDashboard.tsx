...
export default function EnumeratorDashboard() {
  ...
  const [dbSurveys, setDbSurveys] = useState<any[]>([]);
  const navigate = useNavigate();

  useEffect(() => {
    fetch("http://localhost:8080/api/surveys")
      .then(res => res.json())
      .then(data => setDbSurveys(data))
      .catch(err => console.error("Failed to fetch surveys", err));
  }, []);

  const MOCK_NAMES = ["Ramesh Kumar", "Sita Devi", "Mohammed Ali", "Lakshmi N", "Arjun Reddy", "Priya Sharma", "Rahul Verma", "Sunita Patil"];
  const MOCK_LOCATIONS = ["Mumbai, MH", "Patna, BR", "Hyderabad, TG", "Chennai, TN", "Bangalore, KA", "Delhi, DL", "Jaipur, RJ", "Pune, MH"];
  const MOCK_PHONES = ["+91 98453 12345", "+91 99432 96099", "+91 91235 42368", "+91 94894 31540", "+91 98765 43210", "+91 87654 32109", "+91 76543 21098", "+91 65432 10987"];

  const backendRespondents: Respondent[] = dbSurveys.map((s, idx) => ({
    id: `RB-${s.id.substring(0, 4)}`,
    name: MOCK_NAMES[idx % MOCK_NAMES.length],
    phone: MOCK_PHONES[idx % MOCK_PHONES.length],
    location: MOCK_LOCATIONS[idx % MOCK_LOCATIONS.length],
    survey: s.title,
    surveyId: s.id,
    status: s.status === "published" ? "in_progress" : "pending",
    lastContact: "Never",
    progress: 0,
    needsAssistance: false
  }));

  const mergedRespondents = [...backendRespondents, ...respondents];

  const pending = mergedRespondents.filter(r => r.status === "pending").length;
  const followUp = mergedRespondents.filter(r => r.status === "follow_up").length;
  const completed = mergedRespondents.filter(r => ["completed", "assisted"].includes(r.status)).length;
  const needsHelp = mergedRespondents.filter(r => r.needsAssistance).length;

  const filtered = mergedRespondents.filter(r => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
              ...
              <div key={r.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-5 hover:bg-slate-50/50 transition-colors cursor-pointer group">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  {!isCompleted && (
                    <>
                      <button 
                        onClick={() => navigate(r.surveyId ? `/citizen/survey?id=${r.surveyId}` : "/citizen/survey")}
        ...
        <div className="p-4 border-t border-gray-200 bg-slate-50 flex items-center justify-between text-xs text-gray-500">
          <span>Showing {filtered.length} of {mergedRespondents.length} respondents</span>
          <span className="font-bold text-[#1e3a8a]">Response Recovery Rate: {Math.round((completed / Math.max(mergedRespondents.length, 1)) * 100)}%</span>
        </div>
      </div>
    </div>
  );
