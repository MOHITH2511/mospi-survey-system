...
export default function CitizenDashboard() {
  ...
  const backendPending = dbSurveys.filter(s => s.status === "draft").map(s => {
    let qCount = 0;
    try {
      const schema = JSON.parse(s.schemaJson);
      if (Array.isArray(schema)) {
        schema.forEach((sec: any) => qCount += (sec.questions?.length || 0));
      }
    } catch(e) {}
    return {
      id: s.id,
  ...
  const backendLive = dbSurveys.filter(s => s.status === "published").map(s => {
    const images = [householdPoster, economicPoster, healthPoster, literacyPoster];
    const pickedImage = images[s.id % images.length];
    
    return {
      id: s.id,
      theme: "National",
      title: s.title,
      description: s.description || "Recently published survey.",
      image: pickedImage
    };
  });

  const pending = [...backendPending, ...surveys.filter((s) => s.status === "pending")];
  const completed = surveys.filter((s) => s.status === "completed");
  const mergedLiveSurveys = [...backendLive, ...liveSurveys];
          ...
          {mergedLiveSurveys.map((s) => (
            <div key={s.id} className="group flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
              ...
              <div className="p-3 flex-1 flex flex-col">
                ...
                <button
                  onClick={() => navigate(`/citizen/survey?id=${s.id}`)}
