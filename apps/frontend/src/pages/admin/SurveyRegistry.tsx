...
export default function SurveyRegistry() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const mapped = data.map((s: any) => ({
            id: s.id, // Keep full ID for routing
            displayId: s.id.substring(0, 8),
            name: s.title,
            version: `v${s.version || 1}.0`,
            status: s.status === 'draft' ? 'Draft' : s.status === 'published' ? 'Published' : s.status,
            createdBy: 'Current User',
            responses: "-",
            publishedDate: s.createdAt ? s.createdAt.substring(0, 10) : "-",
          }));
          setDbSurveys(mapped);
        }
      } catch (error) {
  ...
  const combinedData = [...registryData, ...dbSurveys];

  const filteredData = activeTab === "All" 
    ? combinedData 
    : combinedData.filter(s => s.status + "s" === activeTab || s.status === activeTab);
                ...
                <tr key={i} className="hover:bg-blue-50/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500 font-mono mt-0.5">{s.displayId || s.id}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        onClick={() => navigate(`/admin/survey-builder?id=${s.id}`)}
                        className="p-2 text-emerald-600 hover:text-emerald-800 hover:bg-emerald-50 rounded" 
                        title="View / Edit"
                      >
